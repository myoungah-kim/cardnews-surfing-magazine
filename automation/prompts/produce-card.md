...
경로가 있다면, 사용자가 Telegram 답장에 직접 첨부한 사진입니다 — 아래 이미지 소싱
순서보다 **최우선**으로 사용하세요. 저작권 걱정이 없으므로 스톡 검색이나 프리셋
배경으로 대체하지 말고, Step 5 절차대로 이 파일을 그대로 배경 이미지 경로에 넣으세요.

## 이 실행에서 반드시 지킬 것

1. **비용이 드는 이미지 생성(AI 생성)을 절대 쓰지 마세요.** 이 실행은 무인이라
   사용자에게 승인을 받을 수 없습니다. 이미지 소싱은 다음 순서로만 진행합니다:
   사용자 첨부 사진(위 항목에 경로가 있을 때) → 원문 이미지(저작권 안전 시) →
   `stock-image-search` 스킬 → `template.css` 프리셋 배경.
