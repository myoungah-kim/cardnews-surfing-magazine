...
| 같은 게시물(이미지·릴즈)이 인스타그램에 두 번 올라감 | Upload 를 짧은 간격으로 두 번 눌러 `finalize-card` 가 큐에 거의 동시에 들어간 경우다(2026-07-31 실사례 — 2초 간격). concurrency 락은 실행 순서만 직렬화할 뿐, 대기 중이던 실행의 checkout 은 큐에 들어간 시점의 커밋에 고정돼 있어 앞선 실행이 그 사이 남긴 게시 기록(permalink)을 못 보고 그대로 다시 게시한다. 뒤의 실행은 git push 충돌로 실행 자체는 실패로 뜨지만 게시는 이미 끝난 뒤라 되돌릴 수 없다 — 중복분은 인스타그램에서 직접 삭제해야 한다. checkout 직후 `git pull --ff-only` 로 재발 방지 조치는 되어 있으니, Upload 는 누른 뒤 결과 메시지가 올 때까지 한 번만 기다릴 것 |
