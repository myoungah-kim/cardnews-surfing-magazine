...
- 배지(`ISSUE` 등 1~3단어) + 헤드라인(2줄 이내) + 서브텍스트(1줄, 간결하게 압축) + 태그
...
1. 첫 줄: 궁금증 유발, 카드 헤드라인과 자연스럽게 이어지는 도입
2. 브리핑 본문: **3문단 이내, 한 문단 2~3줄 이내**로 압축. 육하원칙 중 기사에 실제 있는 것만, 짧은 줄 단위로, 인용문은 원문 그대로 + 출처, 의미/이례성 한두 줄 요약. 가독성을 위해 소제목 사이에 이모지를 적절히 사용
3. 마지막 두 줄: 댓글 유도 + 저장 유도 (분리해서 배치)
4. 출처: 원문 기사 매체명 & 날짜 (예: `Surfer Magazine, 2026.07.23`)
5. 해시태그: 대형(#서핑 #surfing) → 니치 → 브랜드(#서핑슈) 순서
6. **번역 톤**: 해외 아티클을 옮길 때 직역투가 나지 않도록, 국내 언론사 국제부 기자가 쓴 것처럼 자연스러운 한국어 문장으로 작성 (인용문도 의미가 훼손되지 않는 선에서 자연스럽게 다듬어 표기)
...
- `caption.md`가 미니 브리핑 포맷(도입 → 브리핑 본문(3문단 이내) → 댓글유도/저장유도 → 출처 → 해시태그)을 갖추고 있는지
- 캡션 문장이 번역투 없이 자연스러운 한국어(국내 언론 국제부 기자 톤)로 쓰였는지
